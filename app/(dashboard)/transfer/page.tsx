...
interface ValidationSuccess {
  accountName: string;
  txnReference: string;
  type: 'bank' | 'mobile_money' | 'rukapay';
  formData: any;
}
...
export default function TransferPage() {
  ...
  const [rukaPayForm, setRukaPayForm] = useState<RukaPayForm>({
    recipient: "",
    amount: "",
    reason: ""
  });
  ...
  const validateRukaPayAccount = useValidateRukaPayAccount();
  ...
  const handleRukaPayChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setRukaPayForm(prev => ({ ...prev, [name]: value }));
  };
  ...
  // Validate RukaPay account
  const validateRukaPayDetails = async (phoneNumber: string, amount: string) => {
    try {
      const formattedPhone = formatPhoneNumber(phoneNumber);
      const validationData = {
        phoneNumber: formattedPhone
      ...
      const result = await validateRukaPayAccount.mutateAsync(validationData);
      
      // Check if validation was successful
      if (result?.res === 1 && result?.customerDetails?.rdbs_names) {
        setValidationSuccess({
          accountName: result.customerDetails.rdbs_names,
          txnReference: result.txnReference || '',
          type: 'rukapay',
          formData: {
            phoneNumber: formattedPhone,
            amount: Number(amount),
            reason: rukaPayForm.reason,
            merchantId: session?.user?.merchantId
          }
        });
        setShowSuccessPopup(true);
      ...
      console.error("RukaPay validation error:", error);
      throw error;
    }
  };
  ...
  const handleConfirmTransaction = async () => {
    ...
    try {
      ...
      } else if (validationSuccess.type === 'rukapay') {
        // Validate required fields for RukaPay transfer
        if (!validationSuccess.formData.phoneNumber || !validationSuccess.formData.amount) {
          throw new Error("Missing required RukaPay transfer data");
        }

        const rukaPayTransferData = {
          amount: Number(validationSuccess.formData.amount),
          phoneNumber: validationSuccess.formData.phoneNumber,
          reason: validationSuccess.formData.reason || "RukaPay Transfer",
          customerId: merchantId, // Using merchantId as customerId
          customerName: validationSuccess.accountName, // Using the validated account name
        };

        console.log("RukaPay Transfer Data:", rukaPayTransferData);
        const result = await transferMoney.mutateAsync(rukaPayTransferData);
        console.log("transferMoney result========>", result);

        if (result?.res === 1) {
          toast.success(`Transaction completed successfully! Reference: ${result.txnReference || validationSuccess.txnReference}`);
          setSuccess("Transaction completed successfully!");
          setRukaPayForm({ recipient: "", amount: "", reason: "" });
          setShowSuccessPopup(false);
          setValidationSuccess(null);
        } else {
  ...
  // Handle RukaPay form submission
  const handleRukaPaySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError("");
    setSuccess("");
    ...
    try {
      // Validate RukaPay account details first
      await validateRukaPayDetails(rukaPayForm.recipient, rukaPayForm.amount);
      ...
      let errorMessage = "Failed to send money. Please verify the RukaPay account and try again.";
  ...
  const sendFormsContent = [
    // RukaPay Account
    <form key="rukapay" onSubmit={handleSendSubmit} className="space-y-6">
      <div>
        <label className="block text-sm font-medium mb-1">
          Recipient RukaPay Account
        </label>
        <input
          type="text"
          name="recipient"
          value={rukaPayForm.recipient}
          onChange={handleRukaPayChange}
          required
          className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-main-500"
          placeholder="Enter RukaPay Account Number"
        />
      </div>
      <div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-gray-900 capitalize">
                  {validationSuccess?.type === 'bank' ? 'Bank Transfer' : 
                   validationSuccess?.type === 'mobile_money' ? 'Mobile Money' : 
                   validationSuccess?.type === 'rukapay' ? 'RukaPay Transfer' : 'Transfer'}
                </span>
              </div>
              ...
              {(validationSuccess?.type === 'mobile_money' || validationSuccess?.type === 'rukapay') && validationSuccess?.formData?.phoneNumber && (
                <div className="flex justify-between items-center">
